...
# Attack range of the ranged unit
var attackRange = 200
...
func performStateLogic(delta):
	match state:
		...
		states.engaging:
			if parent.attackTarget.get_ref() and parent.targetWithinRange():
				pass
			else:
...
func getTransition(delta):
	match state:
		...
		states.attacking:
			if !parent.attackTarget.get_ref() or parent.targetWithinRange() == false:
				setState(states.idle)
